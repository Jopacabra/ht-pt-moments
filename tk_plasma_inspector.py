...
class MainPage(tk.Frame):
    ...
    def select_file(self, value=None):
        ...
        self.x0Slider.configure(from_=self.current_event.xmax)
        self.x0Slider.configure(to=self.current_event.xmin)
        self.x0.set(0)

        self.y0Slider.configure(from_=self.current_event.ymax)
        self.y0Slider.configure(to=self.current_event.ymin)
        self.y0.set(0)
    ...
    def update_plots(self, value=None):
        if self.file_selected:
            ...
            self.propertyAxes[0, 0].set_title("u_perp")
            self.propertyAxes[0, 1].plot(t, uParArray)
            self.propertyAxes[0, 1].set_title("u_par")
            self.propertyAxes[1, 0].plot(t, tempArray)
            self.propertyAxes[1, 0].set_title("T (GeV)")
            self.propertyAxes[1, 1].plot(t, velArray)
            self.propertyAxes[1, 1].set_title("|u|")
            self.propertyAxes[2, 0].plot(t, (uPerpArray / (1 - uParArray)))
            self.propertyAxes[2, 0].set_title("prp / (1-par)")
            self.propertyAxes[2, 1].plot(t, 1 / (5 * overLambdaArray))
            self.propertyAxes[2, 1].set_title("1/Lmda (fm)")
            self.propertyAxes[1, 2].plot(t, 1 / (overLambdaArray))
            self.propertyAxes[1, 2].set_title("1/Lmda (GeV^-1)")
            self.propertyAxes[2, 2].plot(t, 4 * tempArray ** 2)
            self.propertyAxes[2, 2].set_title("mu^2 (GeV^2)")
            self.propertyAxes[0, 2].plot(t, iIntArray)
            self.propertyAxes[0, 2].set_title("I(k) Factor")
            self.propertyAxes[0, 3].plot(t, XArray)
            self.propertyAxes[0, 3].set_title("X Pos")
            self.propertyAxes[1, 3].plot(t, YArray)
            self.propertyAxes[1, 3].set_title("Y Pos")
            self.propertyAxes[2, 3].plot(t, integrandArray)
            self.propertyAxes[2, 3].set_title("Integrand")
